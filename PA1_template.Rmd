...
```
The result was a single file named `r zippedFileList$Name[1]`. 
...
Inspection of the rawActivity dataframe showed that it consisted of `r nrow(rawActivity)` observations of `r ncol(rawActivity)` variables. The *date* variable was a character variable, not a date variable.   
...
The daily_step_sums dataframe consists of `r nrow(daily_step_sums)` observations corresponding to the total number of different dates for which there was recorded activity data.  
